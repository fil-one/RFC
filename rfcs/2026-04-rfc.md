...
* Permissible status changes include reclassifying an RFC to reflect its maturity or current role over time—for example, `Experimental` → `Standard`, `Standard` → `Historical`. These changes ordinarily do not involve changing the body of the RFC, only the status (but small exceptions are acceptable if agreed in the PR).
* RFCs MAY be updated in place, with the following caveats:
    * Breaking changes to "standard" RFCs MUST be proposed as a new RFC and be accompanied with obsoletes messaging (see below).
    * Backwards compatible changes are permitted for "standard" RFCs but SHOULD be avoided if possible.
    * Changes to "historical" RFCs are not permitted except in exceptional circumstances.
* A "standard" RFC which entirely replaces a prior standard should be marked `Obsoletes: XXX`, and the PR for that RFC should amend the updated RFC to be marked `Obsoleted by: XXX`. These annotations should be hyperlinks.
