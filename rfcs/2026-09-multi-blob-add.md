...
The clients that drive volume know every digest before the first network call. Ingot spools a PutObject to disk and shards it before uploading; any client that pre-shards or erasure-codes an object holds its full digest list up front. Nothing in the protocol lets them say so. And a sharding client cannot express the shape of its redundancy: it needs only enough of its pieces to land, but the protocol makes it drive every blob to completion, one at a time.
...
- The response container carries the upload as executable tasks: each blob's placement is a complete, sprue-signed program, every argument fixed, nothing for a holder to vary — and nothing destructive. Every unwind flows through the upload service under the removal RFC's existing verbs.
...
- The upload service drops off the upload path after planning. It mints the tasks without contacting a node; only cancellation and removal fan out, best-effort, as the removal RFC already does.
...
Today's `/blob/add` response already carries the upload as a program: the `/blob/allocate`, `/http/put` and `/blob/accept` invocations, promise-chained, each executed by a different party. This RFC keeps that contract and widens it. For each blob, sprue pre-mints the full task chain:
...
The allocate and accept invocations are issued and signed by sprue with the provider as subject, under the provider's registration delegation — the same chain nodes validate today; sprue likewise authors the put task under the digest-derived key, and the sprue-subject register task. The agent adds no authority and can vary nothing, only execute or not execute. All four are built without nonces, so their task CIDs are deterministic and known to every party from the moment the response lands, including the register task, whose receipt the client can await from t=0. All four tasks carry an explicit expiry equal to the batch's upload window (the 30-second invocation default would be useless here); a chain registers until that expiry plus the protocol grace period — the registration horizon.

One blob, one node, one program — exactly today's shape. A blob re-placed by `/blob/batch/extend` gets a second program at a different provider; unexecuted programs cost nothing and expire.

The placement carries nothing but the program, and the distinction is the security line: the program's tasks are idempotent and produce only what the space announced, so they are safe to execute on delivery no matter who carries them, while anything destructive — unwinding parked state, releasing accepted state — is a space-authorized verb the client invokes on sprue, which translates it to the node under the [blob-removal RFC](./2026-07-forge-blob-removal.md)'s existing scheme. The client never holds a node-facing artifact whose misuse could hurt anyone.

### Sharding clients and the long tail

Placement is one node per blob; over-provisioning belongs to the client's blob set. A client that erasure-codes or shards announces more piece-blobs than its code needs, drives the PUTs in parallel, and stops driving blobs once enough of its pieces land. Cancelling at enough is client arithmetic across blobs; the abandoned tail is parked state that expires node-side with no message, or is retired eagerly with a digest-scoped `/blob/batch/abort`, which forwards the rejects. A blob whose allocate receipt carries no upload address already holds the bytes (the warm path, unchanged) and lands instantly.

The agent relays `/blob/accept` for the blobs it means to keep, and MUST include the placement's register invocation, put invocation and put receipt in the same container: those artifacts are what the node's own delivery needs, and the node refuses the accept without them.

A blob can hold acceptances at two providers only across `/blob/batch/extend` rounds — the agent re-placed after a presumed failure and the first accept landed anyway. That converges: the registration bound settles one provider, the other learns `Surplus` from its own delivery and reclaims, readers who hit the stale location claim retry the registered provider's claim, and the agent reconciles its index from the register receipts.

### Batch lifecycle

1. **Plan** — `/blob/batch/add`. Sprue selects one node per blob over its provider directory in one pass, mints the placement programs, and returns them. Sprue contacts no node; blobs already registered in the space are returned as such, with no placement.
2. **Allocate, upload, accept** — one container of allocate invocations per node; PUTs in parallel; one container of accept invocations per node. The agent ends the node exchange holding node-signed accept receipts, location claims and PDP promises for every blob that landed. This is the client's completion point.
...
4. **Repair** — a blob whose node failed gets a replacement placement from `/blob/batch/extend` and its receipts are delivered later under the same batch. A batch the client walks away from is cancelled with `/blob/batch/abort`.
...
Announces a batch of blobs for upload. Sprue verifies the caller's space authority and quota exactly as `/blob/add` does, plans placement for the whole batch in one selection pass, and returns one placement per blob, with the placement programs' invocations carried in the response container alongside the provider registration delegations the node-facing invocations need as proofs. Placements go only to nodes that advertised batch support at registration (see [the node legs](#the-node-legs-bloballocate-and-blobaccept-unchanged-verbs)); sprue MUST NOT hand batch placements to a node that has not. Digests already registered in the space are listed in `registered` and receive no placement.
...
type BatchAddOK struct {
  placements [Placement] # one placement per blob
...
4. enforces the registration bound: one acceptance registers per `(digest, space)` — the first delivered wins, and a later acceptance of the same blob at another provider returns a `Surplus` outcome and is not registered;
...
Registration and removal MUST serialize per `(digest, space)`, and two tombstones make their races come out right in both directions. Removing a **registered** blob retires its `/blob/register` task and tombstones its `(digest, space, cause)`: a redelivered chain for that cause returns `Removed`, while a fresh batch's chain — new cause — registers. Removing a digest with **no registration yet** writes a pending tombstone stamped with the removal time, held for the length of the registration horizon: it blocks chains whose placement was minted before the stamp (the batch the client walked away from) and passes chains minted after it (a re-upload). `/blob/batch/abort` writes the same pending tombstones for a whole batch at once. Both checks compare the delivered artifacts — the register task's `cause` and issuance time — against the tombstone record alone, so verifying a delivery needs no record of in-flight uploads.
...
The terminal task of the placement program, and a virtual task in the tradition of `/http/put`: no node serves it. Sprue mints it at `/blob/batch/add` alongside the other three tasks and executes it — issues its receipt — inside the `/blob/accept` conclusion handler when a delivery verifies. The receipt is the canonical, sprue-signed record that a blob is registered — the artifact billing, index reconciliation and disputes start from — and the registration time is the receipt's issue time. Because the task CID is fixed at minting, the client can await this receipt from the moment the batch-add response lands.

Exactly one receipt exists per registered blob, no matter who delivered first or how many times. Removing a registered blob retires its register task; the cause-keyed tombstone records it.
...
Requests a replacement placement for blobs whose node has failed. Each entry MAY exclude providers the client has already failed against — exclusions are client-supplied and SHOULD be cumulative across repair rounds, so a provider that failed in any earlier round stays excluded — and sprue MUST NOT place an entry on an excluded provider. The result is a `BatchAddOK`: fresh placement programs with fresh expiries and a fresh registration horizon for the re-placed blobs, whose quota holds replace the superseded placements' outstanding ones, and whose tasks bind `cause` to the **original** batch-add task, so the whole batch delivers under one cause. Extend MUST NOT issue a placement for a digest carrying a live cause-keyed tombstone under this cause, and extending an aborted batch MUST fail with the named error `BatchAborted`.
...
Cancels a batch's outstanding placements — all of them, or only the digests named in `blobs`: the client renounces the placements that have not registered. A sharding client aborts its over-provisioned tail this way while the rest of the batch stays live. Sprue releases the batch's remaining quota holds and writes the pending tombstones immediately, so chains from the abandoned placements return `Removed` instead of registering — the abort is, in effect, revocation of the outstanding placement programs. Blobs already registered are untouched and listed in the result; they exit via `/blob/remove`.

Node-side cleanup is sprue's translation, exactly as `/blob/abort` translates to `/blob/reject` today: after writing the tombstones, sprue forwards a `/blob/reject` to every renounced placement's provider under its registration delegation, resolving the providers from its stored batch-add receipt. Forwarding is best-effort — the node handler is idempotent, a placement whose allocate never executed rejects as an unknown blob, and node-side expiry reaps whatever a missed forward leaves — and the tombstones land first, so no in-flight delivery races the cleanup. Acceptances that already executed cannot be rejected (`BlobAccepted`); they resolve through the delivery loop — the node concludes, receives `Removed`, and reclaims.

A whole-batch abort terminates the cause: a later `/blob/batch/extend` under it fails with `BatchAborted`, so a cancelled batch cannot mint post-tombstone placements that slip past its own cancellation. A digest-scoped abort terminates only those digests — extend refuses them and the rest of the batch continues.
...
type BatchAbortArguments struct {
  ...
  blobs optional [Bytes] # limit the abort to these digests; absent, the whole batch
}
```
...
type BatchAbortArguments struct {
  Cause cid.Cid               `cborgen:"cause"`
  Blobs []multihash.Multihash `cborgen:"blobs,omitempty"`
}
```
</details>
...
- A sharding client outruns a slow node by finishing other pieces; a blob that must land and cannot costs one `/blob/batch/extend`.
- A refused accept is visible to the agent directly in the node's response; the blob goes back through extend.
...
- Allocations that never receive bytes, and bytes from PUTs the client cancelled or abandoned, expire on the node's existing allocation-expiry hygiene, with no message required. An abandoned batch or tail is cancelled sooner with `/blob/batch/abort`, which forwards the rejects.
...
| 2 | sprue → agent | `BatchAddOK`: one placement program per blob in the container |
| 3 | agent → each node, parallel | one container relaying that node's `/blob/allocate` invocations; receipts carry upload addresses (or none — the node already holds the bytes) |
| 4 | agent → nodes, parallel | HTTP PUTs in parallel; sign a put receipt per landed blob (a sharding client stops at enough pieces) |
| 5 | agent → each node, parallel | one container relaying that node's `/blob/accept` invocations (register and put invocations and put receipts included); receipts carry location claims and PDP promises — **the client is done** |
...
A repair round inserts between legs 5 and 7: the agent calls `/blob/batch/extend` for the blobs whose nodes failed, repeats legs 3–5 against the fresh placements, and the late receipts ride the same delivery path under the original cause. A batch the client abandons is cancelled with one `/blob/batch/abort`; sprue forwards the rejects.
...
- **Bearer execution is bounded by the announcement.** The invocations execute on delivery, so a holder of a leaked response container can advance the program — at worst force-completing an announced upload of content it possesses, billing the space for blobs and sizes the space itself named, inside the expiry window. It cannot divert the program to other digests, sizes, spaces, or nodes; where an extend has left a blob with programs at two providers, a faster delivery can settle the one the agent abandoned, which converges through `Surplus` and reclaim like any late acceptance. `/blob/batch/abort`'s tombstones close the forced-completion sliver for placements the client renounces, whole batches or over-provisioned tails; a tail the client never aborts stays force-completable until its placements expire. Nothing in the container is destructive: an interceptor can only make announced uploads happen, never unwind anyone's state.
...
- **Surplus acceptance is bounded twice.** The registration bound caps what sprue registers per `(digest, space)`; the reclaim deadline — a commitment of every batch-advertising node — returns unregistered acceptances to reclaimable state. An agent that ends up with a second acceptance — possible only across extend rounds — gains nothing durable and costs the nodes storage until reclaim — and, on nodes that defer PDP enqueue to the registration signal, no proof obligations.
...
The cbor-gen list cap (8,192 elements) bounds every list in these schemas. A placement is four signed invocations, a few hundred bytes each, so the batch-add response runs to a couple of megabytes per thousand blobs; the batch cap sizes accordingly. `MaxBlobSize` (256 MiB) is unchanged, enforced node-side at allocate as today.
...
- **The [blob-removal RFC](./2026-07-forge-blob-removal.md)** is amended in three stated places. Its receipt-chain routing walk (the registration's cause, then the add receipt's `site`, then the accept task) does not exist for batch registrations — `BatchAddOK` has no `site` — so for batch-registered blobs routing MUST start from the `/blob/register` receipts, which name the providers and link the accept tasks directly, and `/blob/remove` sends `/blob/release` to the registered provider. Its remove handler gains duties: the pending tombstone on removal of an unregistered digest, and per-`(digest, space)` serialization with registration. And its lifecycle gains the reclaim exit for never-registered acceptances, as specified in [the node legs](#the-node-legs-bloballocate-and-blobaccept-unchanged-verbs). `/blob/batch/abort` is its batch form: the same client-verb-to-`/blob/reject` translation, routed from the stored batch-add receipt instead of a per-blob cause, with node-side expiry as the passive backstop.
...
The batch-add response could carry authority instead of tasks: sprue re-delegates `/blob/allocate` and `/blob/accept` to the agent — subject the provider, policy pinning space, digest, size and cause — and the agent authors the node-facing invocations itself. The two shapes are close: the artifact count per placement is similar, the delivery, horizon, tombstone and advertisement machinery is identical, and nodes validate a provider-rooted chain either way.
...
- **Placement-level replication.** Registering one blob at several providers would reintroduce a replica count, carried by the register task so the registration bound stays readable off the delivered artifact. Out of scope here; the bound in this RFC is one.
...
- The values of the upload window (the tasks' expiry) and the grace period that extends it into the registration horizon; the window wants to be generous, on the order of hours.
...
- Default and maximum sizes for the batch and conclude caps.
...
- Exactly one `/blob/register` receipt exists per registered blob under concurrent node and agent delivery.
