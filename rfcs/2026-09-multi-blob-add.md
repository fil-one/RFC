...
3. **Deliver** — the node SHOULD invoke `/ucan/conclude` on sprue for each accept it executes; the agent SHOULD deliver its collected accept receipts in one batched `/ucan/conclude`. Sprue verifies each delivered chain, registers idempotently, and answers with per-receipt outcomes.
...
4. enforces the registration bound: at most the replica count the placement delegation's metadata names — one, absent the field — is registered per `(digest, space)`. The first delivered acceptance wins; later acceptances of the same blob at other providers return a `Surplus` outcome and are not registered;
5. checks the tombstones (below): a chain matching one returns a `Removed` outcome instead of registering;
6. registers the blob against the space named in the verified chain, with the batch-add task as `cause`, debits the space's quota, and persists the delivered invocations and receipts to the agent store.

Registration and removal MUST serialize per `(digest, space)`, and two tombstones make their races come out right in both directions. Removing a **registered** blob tombstones its `(digest, space, cause)`: a redelivered chain for that cause returns `Removed`, while a fresh batch's chain — new cause — registers. Removing a digest with **no registration yet** writes a pending tombstone stamped with the removal time, held for the length of the registration horizon: it blocks chains whose placement delegation was issued before the stamp (the batch the client walked away from) and passes chains minted after it (a re-upload). Both checks read only the delivered delegation, so verification stays stateless.

Delivery is idempotent and unordered: the node's conclusion and the agent's conclusion of the same accept converge on one registration, and re-delivering is always safe. A delivery failing any check lands in the outcome list with a named reason and blocks nothing else. The legacy `/http/put` conclusion handler is unchanged and continues to serve the single-blob flow.
...
type ConcludeOutcome struct {
  ...
  registered Bool
  reason     optional String # named outcome, e.g. AlreadyRegistered, Surplus, Removed,
                             # Expired, InvalidReceipt, UnknownPlacement
}
...
- **Sprue does only local work per delivery**: verify the chain, apply the horizon, the bound and the tombstones, register, persist. Registration is idempotent, so redundant and repeated deliveries converge, and a delivery can be replayed at any time by anyone still holding the receipts.
...
- The agent SHOULD reconcile its index against its delivery outcomes: a `Surplus` outcome for the receipt it indexed means a different winner registered, and the registered node's location claim is the durable one.
...
- **Receipt-chain consumers**: the persisted chain — `batch add → allocate → put → accept` — has today's shape, so the replay path and the [blob-removal RFC](./2026-07-forge-blob-removal.md)'s receipt-chain routing work unchanged for delivered blobs. Undelivered blobs differ: sprue has not seen their chains, so `/blob/abort` cannot route for them. The backstop is node-side allocation expiry and the reclaim deadline; the optional `/blob/reject` delegation gives an agent early reclaim directly against the node.
...
| 7 | agent → sprue, async | the agent SHOULD deliver its collected receipts in one batched `/ucan/conclude`; the response reports per-receipt registration outcomes |
...
- Whether the agent awaits its conclude response before the S3 200 (stronger registration signal) or fires it after (lower latency); this RFC leans to the latter since resolvability does not depend on it.
